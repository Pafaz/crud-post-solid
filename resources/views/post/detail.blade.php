<x-app-layout class="min-h-max">
    ...
    <div
        ...
        <pre class="text-gray-800 w-full indent-6 whitespace-pre-line break-words overflow-auto text-justify dark:text-gray-300 bg-gray-900 rounded-md p-5 mt-4">
            {{ $post->content }}
        </pre>
    ...
    <div
        ...
        <p class="text-gray-800 two-lines h-full dark:text-gray-300 ">
            {{ $comment->content }}
        </p>

        <form action="{{ route('comments.update', $comment->id) }}" method="post" id="edit-form-{{ $comment->id }}" class="hidden">
            @csrf
            @method('PATCH')
            <textarea id="content-{{ $comment->id }}" name="content" class="resize-none w-full p-2 border rounded-md dark:bg-gray-900 dark:text-gray-300">{{ $comment->content }}</textarea>
            <button type="submit" class="relative -top-9 left-full -ml-12 text-blue-700 hover:underline dark:text-white">Save</button>
        </form>

        @if ($comment->user_id ==  auth()->id())
        <div class="flex items-center justify-end text-sm -mt-7">
            <div class="flex gap-2">
                <button onclick="editComment({{ $comment->id }})" class="text-blue-700 hover:underline dark:text-white">Edit</button>
                <form action="{{ route('comments.destroy', $comment->id) }}" method="post">
                    @csrf
                    @method('DELETE')
                    <button id="button-del-{{ $comment->id }}" type="submit" class="text-blue-700 hover:underline dark:text-white">
                        Delete
                    </button>
                </form>
            </div>
    ...
    {{-- form add comment --}}
    <div>
        <form action="{{ route('comments.store') }}" method="post" id="form-comment">
            @csrf
            <div class="w-3/6 fixed bottom-1 left-0 right-0 mx-auto p-4 bg-white dark:bg-gray-700 dark:border-gray-400/40 border rounded-lg h-40">
                ...
                class="w-full h-full resize-none p-2 border rounded-md dark:bg-gray-900 dark:text-gray-300" placeholder="Add Comment"></textarea>
...
<script>
    ...
    function editComment(commentId) {
        const buttonEditDelete = document.getElementById('button-del-' + commentId);
        const form = document.getElementById(`edit-form-${commentId}`);

        form.classList.toggle('hidden');
        buttonEditDelete.classList.toggle('hidden');
