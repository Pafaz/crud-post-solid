        ...
        <div class="max-w-xl flex w-full flex-col border rounded-lg bg-gray-800 p-8 mt-16">
            <form action="{{ route('posts.store') }}" method="POST">
                ...
                <div class="mb-4 flex flex-col">
                    ...
                    <input type="hidden" name="tags" id="hidden-tags" value="{{ $selectedTags }}">
                </div>
...
<x-app-layout>
    <x-slot name="header">
        ...
        <a href="{{ route('posts.index') }}" class="inline-block px-7 py-1.5 overflow-hidden text-sm font-semibold transition-transform rounded-full group text-white  bg-gray-600 hover:bg-blue-600/70 hover:text-white hover:shadow-lg">
            <span before="Back to Posts" class="relative py-1.5 transition-transform inline-block before:content-[attr(before)] before:py-1.5 before:absolute before:top-full group-hover:-translate-y-full">Back to Posts</span>
        </a>
    </div>

    </x-slot>

    <div class="max-w-xl mx-auto mt-16 flex w-full flex-col border rounded-lg bg-gray-800 p-8 mb-16">
        <form action="{{ route('posts.update', $post->id) }}" method="POST">
        @csrf
        @method('PUT')
        <h2 class="title-font mb-1 text-lg font-medium text-white">Edit Your Post Here</h2>
        <div class="mb-4">
            <label for="title" class="text-sm leading-7 text-white">Title</label>
            <input type="title" value="{{ $post->title }}" id="title" name="title" class="w-full rounded border border-white bg-gray-100 py-1 px-3 text-base leading-8 text-gray-700 outline-none transition-colors duration-200 ease-in-out focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200" />
        </div>
        <div class="mb-4">
            <label for="content" class="text-sm leading-7 text-white">Content</label>
            <textarea id="content"  name="content" class="h-32 w-full resize-none rounded border border-gray-300 bg-white py-1 px-3 text-base leading-6 text-gray-700 outline-none transition-colors duration-200 ease-in-out focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200">{{ $post->content }}</textarea>
        </div>

        <div class="mb-4">
          <label for="category" class="text-sm leading-7 text-white">Category</label>
          <select id="category" name="category" class="w-full rounded border border-gray-300 bg-white py-1 px-3 text-base leading-8 text-gray-700 outline-none transition-colors duration-200 ease-in-out focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200">
              @foreach ($categories as $category)
                  <option value="{{ $category->id }}">{{ $category->name }}</option>
              @endforeach
          </select>
          <button type="button" onclick="showCategoryModal()" class="text-sm leading-7 text-blue-400">Create new Category</button>
      </div>
      
      <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
        @foreach ($tags as $tag)
          <label class="inline-flex items-center">
            <input type="checkbox" name="tags[]" value="{{ $tag->id }}" class="form-checkbox" {{ in_array($tag->id, old('tags', $selectedTags)) ? 'checked' : '' }}>
            <span class="ml-2 text-white">{{ $tag->name }}</span>
          </label>
        @endforeach
      </div>
      ...
      <button type="button" onclick="showModal()" class="text-sm leading-7 text-blue-400">Create new Tag</button>
        ...
        <button type="submit" class="mt-4 inline-block w-full px-7 py-1.5 overflow-hidden text-sm font-semibold transition-transform rounded-full group text-white  bg-gray-600 hover:bg-blue-600/70 hover:text-white hover:shadow-lg">
            <span before="Send Post" class="relative py-1.5 transition-transform inline-block before:content-[attr(before)] before:py-1.5 before:absolute before:top-full group-hover:-translate-y-full">Send Post</span>
        </button>
    </form>
    </div>

    {{-- Create Tag Modal --}}
    <dialog id="tagModal" class="p-6 bg-white rounded-lg shadow-lg w-[400px]">
        <div class="flex justify-between items-center mb-4">
        ...
        <form action="{{ route('tags.store') }}" method="POST">
            ...
            <div>
                ...
                <input type="text" id="tagName" name="name" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" required>
            </div>
            <div class="mt-4">
                <button type="submit" class="inline-block w-full px-4 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700">Create Tag</button>
            </div>
        </form>
    </dialog>

    {{-- Create Category Modal --}}
    {{-- <dialog id="tagModalCategory" class="p-6 bg-white rounded-lg shadow-lg w-[400px]">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-bold">Create New Tag</h2>
            <button onclick="closeTagModal()" class="text-gray-600 hover:text-gray-900 text-2xl">&times;</button>
        </div>
        <form action="{{ route('categories.store') }}" method="POST">
            @csrf
            <div>
                <label for="categoryName" class="block text-sm font-medium text-gray-700">Category Name</label>
                <input type="text" id="categoryName" name="categoryName" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" required>
            </div>
            <div>
                <label for="description" class="block text-sm font-medium text-gray-700">Category Name</label>
                <textarea type="text" id="description" name="description" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" required>
            </div>
            <div class="mt-4">
                <button type="submit" class="inline-block w-full px-4 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700">Create Tag</button>
            </div>
